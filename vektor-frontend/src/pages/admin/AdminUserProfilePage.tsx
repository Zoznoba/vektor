...
import { fetchUsers, fetchChildren, assignChildren, setUserActive } from '../../api/users';
 ...
 * URL, и подсветка `users`). `/admin/users/:id/results` сохранён как синоним
 * `/admin/users/:id` — по нему ведут действия «Диагностика» из «Классов»,
 * «Кейсов» и покрытия кампании, и ломать их незачем. Разного поведения у двух
 * адресов нет: блок диагностики раскрыт в обоих случаях.
 *
  ...
  return (
    <AdminShell activeNavKey="users">
      ...
      {users.loading && !users.data ? (
        ...
        <Panel>
          <div className="admin-empty">Пользователь не найден — возможно, его уже удалили</div>
        </Panel>
      ) : (
        <>
          <Panel>
            <div className="user-card">
              ...
              <div className="user-card__actions">
                ...
                {user.is_active ? (
                  ...
                  <Button variant="secondary" onClick={handleActivate} disabled={submitting}>
      ...
      {user && resetPasswordOpen && (
        <ResetPasswordModal user={user} onClose={() => setResetPasswordOpen(false)} />
      )}
