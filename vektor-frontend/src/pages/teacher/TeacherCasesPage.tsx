...
/**
 * Средний профиль кейса против школы — тот же блок, что у админа на
 * `/admin/cases` и у класса на экране диагностики: различаются только
 * подписи и источник данных.
 *
...
function CaseProfilePanel({ caseId, caseName }: { caseId: number; caseName: string }) {
  ...
  const load = useCallback(
    async () => caseResultsToAnalytics(await fetchCaseResults(caseId)),
    [caseId],
  );
  const loadDynamics = useCallback(() => fetchGroupDynamics('case', caseId), [caseId]);
  ...
  return (
    <Panel title="Профиль кейса">
      <GroupAnalytics
        ...
        emptyText="Диагностики по этому кейсу ещё не было — профиль появится после первой завершённой кампании."
      />
    </Panel>
  );
}
