...
/**
 * Сайдбар учителя. «Главной» и «Профиля» у учителя нет намеренно: стартовый
 * экран — «Мои классы», а личных результатов у учителя не бывает (он не
 * субъект оценки, а оценивающий).
 *
 * Отдельного пункта «Профиль ученика» тоже нет: к результатам конкретного
 * ученика ведут строки состава в «Моих классах» (диагностика класса) и в
 * «Моих кейсах» — третий список тех же учеников в сайдбаре только дублировал
 * бы их. Экран `/teacher/students/:id` остаётся, но открывается только оттуда.
 */
 ...
 * (`/admin/users/:id`). Заодно ушла путаница с «Сводкой»: теперь
