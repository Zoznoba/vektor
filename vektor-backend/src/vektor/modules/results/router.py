...
@router.get(
    ...
    "кампания, где участвовал кто-то из нынешних учеников класса; по "
    "незавершённой результаты не отдаются (409). Архив прошлых когорт той же "
    "строки класса открывается явным campaign_id — список даёт "
...
async def get_class_results(
    ...
    # Резолюция кампании — в сервисе: «последняя кампания класса» должна
    # учитывать нынешний состав (см. _resolve_class_campaign), а состав
    # сервис и так грузит ради прав. У кейса такого условия нет — см. ниже.
...
@router.get(
    ...
    "баллы без дельт. Кампания по умолчанию выбирается так же, как у профиля "
    "класса. Доступно админу и учителю этого класса.",
)
