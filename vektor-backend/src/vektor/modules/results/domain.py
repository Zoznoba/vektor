...
from vektor.shared.enums import AssessmentBasis, RaterRole, UserRole
...
def coverage_key(
    class_id: int | None,
    case_id: int | None,
    issued_for: AssessmentBasis | None = None,
) -> CoverageKey:
    """Строка покрытия, в которую попадает анкета — по ОСНОВАНИЮ её выдачи
    (Assessment.issued_for), а не по наличию снапшотов.
    ...
    Почему не по снапшотам. subject_class_id проставлен у КАЖДОЙ анкеты (от
    него зависит видимость возрастных вопросов), в том числе у выданной за
    кружок, поэтому группировка по классу растворяла бы кейс в классах его
    участников. Обратное правило «есть кейс — значит кейсовая» врало
    симметрично: у ученика, состоящего и в классе, и в кейсе, пары обоих
    оснований сливаются в ОДНУ анкету (merge_pairs), и в строку кейса уезжала
    вся его диагностика — самооценка, родители, учителя класса, — а из своего
    класса ученик пропадал вовсе. Теперь самооценка, родители и учителя
    класса остаются в строке класса, а руководители кружка — в строке кейса,
    и ученик виден в обеих.

    Основание не проставлено (анкета старше миграции a4d61c30f9b2, backfill
    которой не нашёл ни одного снапшота) — падаем на прежнее правило: строка
    кейса, потом класса, потом «без класса». Сумма строк равна итогу кампании
    при любой ветке: анкета попадает ровно в одну.
    """
    if issued_for == AssessmentBasis.CASE and case_id is not None:
        return ("case", case_id)
    if issued_for == AssessmentBasis.CLASS and class_id is not None:
